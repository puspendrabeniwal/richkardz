...
round = (value, precision) => {
  try {
    if (!value || isNaN(value)) {
      ...
      precision =
        typeof precision != typeof undefined && precision ? precision : NOT;
...
addDaysToDate = (hours, date) => {
  ...
  let baseTimestamp =
    typeof date !== typeof undefined && date
...
convertObjectToArray = (obj) => {
  ...
  let newArray = Object.keys(obj).map((objKey) => {
    return obj[objKey];
  });
  return newArray;
};
  ...
  return new Promise((resolve) => {
    ...
    users.findOne(conditions, { projection: fields }, (err, result) => {
      ...
      if (!result.profile_image)
        return resolve({ status: STATUS_SUCCESS, result: result });
      ...
      appendFileExistData(imageOptions).then((fileResponse) => {
        ...
        resolve({
          status: STATUS_SUCCESS,
          result:
            fileResponse && fileResponse.result && fileResponse.result[0]
              ? fileResponse.result[0]
              : {},
        });
      });
    });
